...
class DataInterface
{
	...
	public function getPlayerIds($teamId)
	{
		//get all the game ids associated with a team
		$conn = $this->conn;
		$result = $conn->query("SELECT pid FROM Player WHERE teamId=".$teamId."");
		if ($result->num_rows > 0) {
	    // output data of each row
		$arr = Array();
	    while($row = $result->fetch_assoc()) {
	...
	{
		$conn = $this->conn;
		$result = $conn->query("SELECT team1,team2 FROM Game WHERE gameId='".$gameId."'");
		if ($result->num_rows > 0) {
		    // output data of each row
		    while($row = $result->fetch_assoc()) {
		        $team1 = $row["team1"];
		...
		$game = new Game($team1, null, $team2, null, null, null, null);
		$game->setID($gameId);
		return $game;
	...
	public function getStat($statType, $pid, $gameId, $setNum, $playerScoreMin, $playerScoreMax, $opponentScoreMin, $opponentScoreMax, $scoreDifferenceMin, $scoreDifferenceMax)
	{
		...
		if(($gameId>0) && ($setNum>0)) {
			$result = $conn->query("SELECT value FROM Stat WHERE (pid='".$pid."') AND (statType='".$statType."') AND (gameId='".$gameId."') AND (setNum='".$setNum."')");
		}
		else if($gameId>0){
			$result = $conn->query("SELECT value FROM Stat WHERE (pid='".$pid."') AND (statType='".$statType."') AND (gameId='".$gameId."')");
		}
		else if($setNum>0){
			$result = $conn->query("SELECT value FROM Stat WHERE (pid='".$pid."') AND (statType='".$statType."') AND (setNum='".$setNum."')");
		}
		else{
			$result = $conn->query("SELECT value FROM Stat WHERE (pid='".$pid."') AND (statType='".$statType."')");
		}

		/*
		if(($pid>0) && ($gameId>0) && ($setNum>0)) {
		...
		else if(($pid>0) && ($gameId>0)) {
			$result = $conn->query("SELECT value FROM Stat WHERE pid='".$pid."'AND gameId='" .$gameId."'AND setNum='*'AND statType='".$statType."'AND playerScore>='".$playerScoreMin."'AND playerScore<='".$playerScoreMax."'AND opponentScore>='".$opponentScoreMin."'AND opponentScore<='".$opponentScoreMax."'AND (playerScore-opponentScore)>='".$scoreDifferenceMin."'AND (playerScore-opponentScore)<='".$scoreDifferenceMax."'");
		}
		else if(($pid>0) && ($setNum>0)) {
		...
		*/
		...
		return new Stat($statType,$total);
	}
}
?>