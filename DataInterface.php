...
class DataInterface
{
	...
	public function getStat($statType, $pid, $gameId, $setNum, $playerScoreMin, $playerScoreMax, $opponentScoreMin, $opponentScoreMax, $scoreDifferenceMin, $scoreDifferenceMax)
	{
		...
		if(isset($pid) && isset($gameID) && isset($setNum)) {
			...
			$result = $this->conn->query("SELECT value FROM Stat WHERE statType=".$statType."playerScore>=".$playerScoreMin."AND playerScore<=".$playerScoreMax."AND opponentScore>=".$opponentScoreMin."AND opponentScore<=".$opponentScoreMax."AND (playerScore-opponentScore)>=".$scoreDifferenceMin."AND (playerScore-opponentScore)<=".$scoreDifferenceMax."");
		}
		$total = 0;
		if ($result->num_rows > 0) {
		    // output data of each row
